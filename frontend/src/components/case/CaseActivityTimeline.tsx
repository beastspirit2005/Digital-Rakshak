              ...
              <div className="flex justify-between items-start mb-2">
                ...
                {event.timestamp && (
                  <span className="text-xs text-ink-3">
                    {new Date(event.timestamp.endsWith("Z") || event.timestamp.includes("+") ? event.timestamp : event.timestamp + "Z").toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "numeric", hour12: true })}
                  </span>
                )}
              </div>
